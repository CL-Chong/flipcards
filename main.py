...
def question_generator(qname):
    ...
    if qname == "quickdiff0":
        ...
        w = np.random.randint(1,9) * x ** Fraction(np.random.randint(1,5),np.random.randint(1,4))
        ...
        U = np.random.choice([sin(x), cos(x), exp(x), log(x)], replace = False, size = 2)
        w = np.random.randint(0, 9) * x ** Fraction(np.random.randint(1, 5), np.random.randint(1, 4))

        fcn = np.random.choice([U[0] * (U[1] ** -1), U[0] * (w ** -1), w * (U[0] ** -1)])
        question = "$\\frac{{d}}{{dx}}\\left(" + latex(fcn) + "\\right)$"
        dfcn = expand(diff(fcn, x))
        answer = "$" + latex(dfcn) + "$"
        ...
        dfcn = diff(fcn, x)
        answer = "$" + latex(dfcn) + "$"
    elif qname == "diffchain2b": # chain rule 2b - polynomial inside elementary
        A = np.random.randint(0, high=6, size=[2])
        B = np.random.randint(-9, high=9, size=[2])
        if B[0] == 0 and B[1] == 0:
            B[0] = B[0] + 1
        ...
        fcn = np.random.choice([sin(g), cos(g), exp(g), log(g2)])
        question = "$\\frac{{d}}{{dx}}\\left(" + latex(fcn) + "\\right)$"
        dfcn = diff(fcn, x)
        answer = "$" + latex(dfcn) + "$"
...
listq = [
    "diffchain2b", # chain rule 2b - polynomial inside elementary
